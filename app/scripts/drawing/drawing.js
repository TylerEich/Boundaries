...
angular.module('boundaries.drawing', ['ngStorage', 'boundaries.map', 'boundaries.color'])
  .service('DirectionsSvc', function($q, MapSvc) {
    ...
    this.route = function(locations) {
      var requests = [];
      var chunkLength = 10;
      var i, j, chunk, start, end, waypoints;
      ...
      var finalPromise = $q.all(promises).then(function(paths) {
        var combinedPaths = Array.prototype
          .concat.apply(paths[0], paths.slice(1));
  ...
  .service('DrawingSvc', function($q, $localStorage, DirectionsSvc, MapSvc, ColorSvc) {
    ...
    this.makeNode = function(colorIndex, latLng) {
      ...
      return {
        lat: latLng.lat(),
        lng: latLng.lng(),
        // TODO: replace -1 with real index, calculated by makePath
        index: -1,
        _marker: marker
      };
    };
    this.spliceNode = function(drawingIndex, nodeIndex, removeLength, newNodes) {
      ...
      var removed = Array.prototype
        .splice.apply(this.drawings[drawingIndex], args);

      // Remove obselete nodes from map
      ...
      for (i = 0; i < changedNodes.length; i++) {
        ...
        if (i <= 1) {
          ...
          // Value of rigid changed => different algorithm
          promises.push(this.makePath(locations));

          // Preserve last location as start of new locations
    ...
    this.spliceDrawing = function(drawingIndex, removeLength, newDrawings) {
      ...
      var removed = Array.prototype
        .splice.apply(this.drawings, args);
